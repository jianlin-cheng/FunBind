...
def compute_similarity(group, model, modality):
    ...
    for protein in protein_list:
        ...
        if modality == 'Sequence':
            modality_embeddings.append(embedding[modality][model_map[modality]])
        else:
            modality_embeddings.append(embedding[modality])
    modality_embeddings = torch.stack(modality_embeddings, dim=0).to(device)
    ...
    ontology_embeddings = torch.cat(ontology_embeddings, dim=0).to(device)


    with torch.no_grad():
        mod1_features, _ = model.encode_modality(modality=f'{modality}_modality', value=modality_embeddings.squeeze(1))
    ...
    results = (ret_1.item(), ret_3.item(), ret_5.item(), ret_mrr.item())
...
def plot(datasets):
    ...
    plt.savefig(f"retrival_no_grouping.png")
...
def run_experiment(ontology=None):
    ...
    for modality in modalities:
        ...
        print(f"Ontology {ontology}, Modality {modality}")
        ...
        if modality != "Sequence":
            datalist = {key: value for key, value in datalist.items() if key != "Sequence"}
            datalist = set(datalist[modality])
        ...
        results[modality] = compute_similarity(group=groups[0], model=model, modality=modality)
