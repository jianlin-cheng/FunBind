...
# Used for Table 2 & 3 in the paper
...
def collect_data(ontology=None):
    ...
    for ontology in ontologies:
        pth = f"{BASE_DATA_DIR}/data/labels/{ontology}_groundtruth"
        data = load_pickle(pth)
        training_terms = set([term for go_terms in data.values() for term in go_terms])

        data_test = load_pickle(BASE_DATA_DIR + "/test/test_data")[ontology]
...
def compute_similarity(group, model, weights):
    ...
    for modality in model_map:
        ...
        for protein in protein_list:
            embedding = torch.load(f'{BASE_DATA_DIR}/test/dataset/{protein}.pt')
            if modality == 'Sequence':
                tmp_embeddings.append(embedding[modality][model_map[modality]])
            else:
                tmp_embeddings.append(embedding[modality])
        all_embeddings[modality] = torch.stack(tmp_embeddings, dim=0).to(device)
    ...
    all_embeddings["Ontology"] = ontology_embeddings
    ...
    combined_features = features["Sequence"] * weights[0] + \
                        features["Structure"] * weights[1] + \
                        features["Text"] * weights[2] + \
                        features["Interpro"] * weights[3]
    
    ret_1 = retrieve_at_1(modality1_features=combined_features, modality2_features=features["Ontology"], groundtruth_all_indices=get_same_position_indices(terms))
    ret_3 = retrieve_at_3(modality1_features=combined_features, modality2_features=features["Ontology"], groundtruth_all_indices=get_same_position_indices(terms))
    ret_5 = retrieve_at_5(modality1_features=combined_features, modality2_features=features["Ontology"], groundtruth_all_indices=get_same_position_indices(terms))
    ret_mrr = retrieve_mrr(modality1_features=combined_features, modality2_features=features["Ontology"], groundtruth_all_indices=get_same_position_indices(terms))


    similarity = (50.0 * combined_features @ features["Ontology"].T).softmax(dim=-1)
    return similarity.cpu(), terms, protein_list, {"Ret@1": ret_1.item(), "Ret@3": ret_3.item(), "Ret@5": ret_5.item(), "MRR": ret_mrr.item()}


def run_experiment(ontology=None, step_size=3, i=0, weights=None):

    datalist = load_pickle(BASE_DATA_DIR +"/test/test_data_modality")
    ...
    if i == 9:
        ...
        for start_idx in range(0, num_groups, step_size):
            ...
            for idx, group in enumerate(subgroup):
                ...
                sim, Xs, Ys, metrics = compute_similarity(group=group, model=model, weights=weights)
                ...
                print(f"Group {idx + 1} - Ret@1: {metrics['Ret@1']}")

                i, j = divmod(idx, cols)

                if step_size == 12:
                    im = sns.heatmap(sim, xticklabels=Xs, yticklabels=Ys, cmap='YlOrRd', ax=axes[i, j])
                    axes[i, j].set_xticklabels(axes[i, j].get_xticklabels(), rotation=45, ha='right', rotation_mode="anchor", fontsize=20)
                    axes[i, j].set_yticklabels(axes[i, j].get_yticklabels(), rotation=45, ha='right', rotation_mode="anchor", fontsize=20)
            ...
            fig.supxlabel("GO Terms", fontsize=24)
            fig.supylabel("Proteins", fontsize=24)

            plt.tight_layout()

            if ontology:
                plt.savefig(f"{ontology}_{modality}_{start_idx + 1}_{end_idx}.png")
            else:
                plt.savefig(f"{modality}_{start_idx + 1}_{end_idx}.png")
            
        print("Length is:", len(all_metrics["Ret@1"]))
        all_metrics = {key: sum(value) / len(value) for key, value in all_metrics.items()}
        return all_metrics


def run_n_times(ontology, n=10, weights=None):

    all_runs_metrics = defaultdict(list)
    for i in range(n):
        print("Run {}...".format(i + 1))
        # step_size = 3 if generate plot in main paper else supplementary
        # Use i == 10 to generate
        metrics = run_experiment(ontology=ontology, step_size=12, i=i, weights=weights)
        #for key, value in metrics.items():
         #   all_runs_metrics[key].append(value)

     # Calculate and print mean and standard deviation in a table format
    print(f"\nResults for {ontology} (Mean +/- SD):")
    print("-" * 30)
...
if not ontology:
    ...
    weights = weights[ontology]

print(weights)
run_n_times(ontology=ontology, n=10, weights=weights)
