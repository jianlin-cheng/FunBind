    ...
    def __getitem__(self, idx):
        ...
        if self.modality == "Fused":
            ...
            if self.modality == "Sequence":
                modality = sample[self.modality]['prostt5']
            else:
...
def load_model(ontology, base_model, device):
    ...
    ckp_file = ckp_dir + f"{ontology}_{base_model}.pt"
...
def main():
    ...
    data_path = BASE_DATA_DIR + "/test/ck"  
    ...
    model = load_model(ontology=ontology, base_model=base_model, device=device)
    model.eval()
    ...
    for mod, protein_predictions in predictions_dict.items():
        with open(f"{BASE_DATA_DIR}/evaluation/predicss/{ontology}/{mod}_{base_model}.tsv", "w") as f:
